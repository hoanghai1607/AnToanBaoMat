...
using namespace std;
void SplitUlonglong(unsigned long long input, unsigned int& part1, unsigned int& part2)
{
    part1 = input >> 32;    // L?y 32 bit cao nh?t
    part2 = input & 0xFFFFFFFF; // L?y 32 bit th?p nh?t
}

// Hàm ð? chia m?t giá tr? 32-bit thành hai ph?n 16-bit
...
string ByteToString(unsigned int b)
{
    std::stringstream ss;
...
string ShowByte(unsigned int C)
{
    string ans;
    for(int i = 1; i <= 8; i++)
    {
        unsigned int b = (C >> (32 - 4*i)) & 0xF;
        cout <<uppercase<< hex << b;
        ans += ByteToString(b);
    }
    return ans;
}
...
void MahoaDES(unsigned int M1, unsigned int M2, unsigned int K1, unsigned int K2, unsigned int &C1, unsigned int &C2)
{
    ...
    for(int i=0; i<16; i++)
        cout << "\nKhoa thu " << i + 1 << ": " << hex << key1[i] << " " << hex << key2[i];
    //Buoc 3. Thuc hien hoan vi
...
int main()
{
    ...
    cout << "Nhap gia tri cho K (hexadecimal, 16 chu so): ";
    cin >> hex >> K;

    cout << "Nhap gia tri cho M (hexadecimal, 16 chu so): ";
    cin >> hex >> M;

    // Chia K thành K1 và K2
    SplitUlonglong(K, K1, K2);

    // Chia M thành M1 và M2
